...
if command -v bat &>/dev/null; then
  ...
  export BAT_THEME="Catppuccin-mocha"        # change to your taste
else
...
if command -v fzf &>/dev/null; then
  ...
  # Preview with bat, colors, layout
  ...
  if [[ -f "$HOME/.fzf.zsh" ]]; then
    ...
    for _fzf_root in /opt/homebrew/opt/fzf /usr/local/opt/fzf; do
...
if [[ -f "$_zas" ]]; then
  ...
  bindkey '^]' autosuggest-accept     # → also accepts (fallback)
fi

# zsh-syntax-highlighting — fish-like syntax coloring
_zsh=/opt/homebrew/share/zsh-syntax-highlighting/zsh-syntax-highlighting.zsh
if [[ -f "$_zsh" ]]; then
  source "$_zsh"
...
# zsh-history-substring-search — ↑/↓ to search history by prefix
_zhss=/opt/homebrew/share/zsh-history-substring-search/zsh-history-substring-search.zsh
if [[ -f "$_zhss" ]]; then
  source "$_zhss"
...
fcd() {
  ...
  dir=$(fd --type d --hidden --exclude .git 2>/dev/null \
        | fzf --preview 'ls -la {}' --prompt '  ') && cd "$dir"
}
...
# hidden — toggle hidden files in Finder (macOS)
hidden() {
  local state
...
export PATH="/Users/mostofamorshed/.antigravity/antigravity/bin:$PATH"
